...
export default async (req: Request, context: Context): Promise<Response> => {
  ...
  const targetHost = isSocketRequest 
    ...
    : "sample-project-core.sample-project.svc.cluster.local";
  const targetPort = isSocketRequest ? 8999 : 3000;
  ...
  return new Promise((resolve) => {
    ...
    proxyReq.on("connect", (res, socket, head) => {
      ...
      const targetOptions = {
        ...
        hostname: targetHost,
        port: targetPort,
        path: isSocketRequest ? "/socket" : "/api", // Ensure the correct path is used
      };
